...
import interpreter.bytecode.debuggerByteCodes.HeaderCode;
...
/**
 * DebugVirtualMachine allows an X program to be debugged. 
 * @author Ramin
 */
public class DebugVirtualMachine extends VirtualMachine {
    private Vector<LineEntry> lineEntries;
     ...
     */
    public DebugVirtualMachine(Program p, Vector<LineEntry> vs) {
        super(p);
        lineEntries = vs;
...
//-------------------------------New Debug functions-----------------------------------
    /**
     * Test if the DebugVirtualMachine is currently 
    ...
    public String OutputTraceEntry() {
        ...
        int size = runStack.getCurrentFrameSize();
    ...
    /**
     * Clear all break points on the DebugVirtualMachine.
     */
    public void ClearAllBkpts() {
        for (LineEntry ent: lineEntries)
            ent.clrBreakpt();
    }
    /**
    ...
    public String showBreakpts() {
        ...
        for (int i = 0; i < lineEntries.size(); i++) {
            LineEntry ent = lineEntries.elementAt(i);
            if (ent.hasBreakpt()) 
                bkpts += (i+1) + " ";
        }
        return bkpts;
    ...
    /**
     * Set the DebugVirtualMachine to step out of the current function.
     */
    public void stepOut() {
        step = new StepOut(records.size());
    }
    /**
     * Set the DebugVirtualMachine to step over a line. 
     */
    ...
    /**
     * Set the DebugVirtualMachine to step into a function.
     */
    public void stepIn() {
        int size = records.size();
        int line = records.peek().getCurrentLine();
    ...
    /**
     * Set the current function on the DebugVirtualMachine,
    ...
    public void addLocal(String id) {
        addSymbol(id, runStack.getCurrentFrameSize() - 1);
    }
    /**
